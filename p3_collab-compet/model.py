...
class MADDPGCritic(nn.Module):
    """Centralized critic: receives concatenated states+actions of all agents."""

    def __init__(self, full_state_size, full_action_size, seed, fcs1_units=400, fc2_units=300):
    ...
    def forward(self, state_all, action_all):
        xs = F.relu(self.bn1(self.fcs1(state_all)))
        x = torch.cat((xs, action_all), dim=1)
        x = F.relu(self.fc2(x))
        return self.fc3(x)
